...
namespace Home
{
    public class Zipcode
    {
        ...
        private static void showPostOffice(JObject response)
        {
            ...
            Console.WriteLine($"Number of postoffice(s) found in this pincode: {postOffices!.Count}");
            foreach (var postOffice in postOffices!)
            {
                string address = $"{postOffice["Name"]}, {postOffice["District"]}, {postOffice["State"]}.";
                Console.WriteLine(address);
            }
        }
        private static async Task findPlace(int code){
            HttpClient httpClient = new HttpClient();
            var responseTask = await httpClient.GetAsync($"https://api.postalpincode.in/pincode/{code}");
